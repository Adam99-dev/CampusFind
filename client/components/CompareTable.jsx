...
const CompareTable = ({ colleges }) => {
  ...
  const BestBadge = ({ metric, className = "" }) => (
    <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-500 text-white text-xs font-semibold ${className}`}>
  ...
  const rows = [
    ...
    },
    {
      key: "campusArea",
      label: "Campus Area",
      icon: <LandPlot className="w-4 h-4 text-blue-600" />,
      ...
      render: (c, idx) => {
        ...
        return (
          <div className="relative">
            <span className={`text-sm ${isBest ? 'text-blue-700 font-bold' : 'text-gray-700'}`}>
                  ...
                  >
                    {isOverallBest && (
                      <div className="absolute -top-2 left-1/2 transform -translate-x-1/2">
                        <div className="bg-blue-500 text-white text-xs font-bold px-2 py-0.5 rounded-full flex items-center gap-1 shadow-lg">
                        </div>
                      </div>
                    )}
                    {abbr ? (
